...
class MatchingAgent:
    ...
    def match(self, ticket: Dict[str, Any], donation_data: Dict[str, Any],
        ...
        try:
            ...
            logger.info("Matching Agent: Fetching volunteers and NGOs from backend")
            ...
            if not volunteers:
                logger.error("Matching Agent: No volunteers returned from backend")
                raise Exception("No available volunteers found")
            if not ngos:
                logger.error("Matching Agent: No NGOs returned from backend")
                raise Exception("No available NGOs found")
            
            # Get pickup location coordinates
            pickup_location = donation_data.get("pickup_address")
            logger.info(f"Matching Agent: Geocoding pickup address: {pickup_location}")
            pickup_coords = self._geocode_address(pickup_location)
            
            if not pickup_coords:
                logger.error(f"Matching Agent: Failed to geocode pickup address: {pickup_location}")
            ...
            if not best_volunteer or not best_ngo:
                ...
                if not volunteer_rankings:
                    logger.error(f"Matching Agent: No volunteers found within {max_distance} km of pickup location {pickup_coords}")
                if not ngo_rankings:
                    logger.error(f"Matching Agent: No NGOs found within {max_distance} km with sufficient capacity")
                raise Exception("Could not find suitable matches - no volunteers or NGOs available within distance constraints")
            ...
            # Log detailed matching decision
            logger.info(
                f"Matching Agent: Assigned Volunteer {best_volunteer['id']} "
                f"(distance: {best_volunteer.get('distance_km')} km, score: {best_volunteer.get('score', 0):.2f}) "
                f"and NGO {best_ngo['id']} "
                f"(distance: {best_ngo.get('distance_km')} km, score: {best_ngo.get('score', 0):.2f}, "
                f"capacity: {best_ngo.get('capacity', 0)})"
            )
            logger.info(f"Total candidates considered: {len(volunteers)} volunteers, {len(ngos)} NGOs")
            logger.info(f"Final rankings: {len(volunteer_rankings)} volunteers, {len(ngo_rankings)} NGOs within constraints")
    ...
    def _rank_volunteers(self, volunteers: List[Dict[str, Any]], 
        ...
        Assumes all volunteers have latitude/longitude (enforced by backend)
        """
        max_distance = ticket.get("constraints", {}).get("max_distance_km", DEFAULT_MAX_DISTANCE_KM)
        ...
        for volunteer in volunteers:
            # Get volunteer coordinates directly (no geocoding needed)
            latitude = volunteer.get("latitude")
            longitude = volunteer.get("longitude")
            
            # Enforce location presence - fail if missing
            if latitude is None or longitude is None:
                logger.warning(f"Volunteer {volunteer.get('id')} missing location data - skipping")
                continue
            
            try:
                volunteer_coords = (float(latitude), float(longitude))
            except (ValueError, TypeError) as e:
                logger.warning(f"Volunteer {volunteer.get('id')} has invalid coordinates: {e} - skipping")
                continue
            ...
            if distance_km > max_distance:
                logger.debug(f"Volunteer {volunteer.get('id')} skipped - distance {distance_km:.2f} km exceeds max {max_distance} km")
                continue
        ...
        logger.info(f"Ranked {len(scored_volunteers)} volunteers within {max_distance} km")
    ...
    def _rank_ngos(self, ngos: List[Dict[str, Any]], pickup_coords: tuple,
        ...
        Assumes all NGOs have latitude/longitude (enforced by backend)
        """
        max_distance = ticket.get("constraints", {}).get("max_distance_km", DEFAULT_MAX_DISTANCE_KM)
        ...
        for ngo in ngos:
            # Get NGO coordinates directly (no geocoding needed)
            latitude = ngo.get("latitude")
            longitude = ngo.get("longitude")
            
            # Enforce location presence - fail if missing
            if latitude is None or longitude is None:
                logger.warning(f"NGO {ngo.get('id')} missing location data - skipping")
                continue
            
            try:
                ngo_coords = (float(latitude), float(longitude))
            except (ValueError, TypeError) as e:
                logger.warning(f"NGO {ngo.get('id')} has invalid coordinates: {e} - skipping")
                continue
            ...
            if distance_km > max_distance:
                logger.debug(f"NGO {ngo.get('id')} skipped - distance {distance_km:.2f} km exceeds max {max_distance} km")
                continue
            
            # Check capacity
            capacity = ngo.get("capacity", 0)
            if capacity <= 0:
                logger.debug(f"NGO {ngo.get('id')} skipped - capacity {capacity} is insufficient")
                continue
        ...
        logger.info(f"Ranked {len(scored_ngos)} NGOs within {max_distance} km with sufficient capacity")
        return scored_ngos
